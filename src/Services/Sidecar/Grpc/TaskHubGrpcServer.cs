...
/// <summary>
/// A gRPC server that implements the <see cref="P.TaskHubSidecarService.TaskHubSidecarServiceBase"/> service contract.
/// </summary>
public class TaskHubGrpcServer : P.TaskHubSidecarService.TaskHubSidecarServiceBase, ITaskExecutor, IDisposable
{
    ...
    public override async Task<P.CreateInstanceResponse> StartInstance(P.CreateInstanceRequest request, ServerCallContext context)
    {
        ...
        this.log.CreatingNewInstance(instance.InstanceId);
    ...
    {
        this.log.RaisingEvent(request.InstanceId, request.Name);
    ...
    public override async Task<P.TerminateResponse> TerminateInstance(
        P.TerminateRequest request,
        ServerCallContext context)
    {
        this.log.TerminatingInstance(request.InstanceId);
    ...
    /// <param name="context">Context for the server-side gRPC call.</param>
    /// <returns>Returns an empty ack back to the remote SDK that we've received the completion.</returns>
    public override Task<P.CompleteTaskResponse> CompleteActivityTask(
        P.ActivityResponse request,
        ServerCallContext context)
    {
        ...
        lock (this.isConnectedSignal)
        {
            outputStream = this.workerToClientStream ??
                throw new InvalidOperationException(
                    "No client is connected! Need to wait until a client connects before executing!");
        }
    ...
    {
        return string.Concat(instanceId, "__", taskId.ToString(CultureInfo.InvariantCulture));
    }
