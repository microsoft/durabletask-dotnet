...
For a deeper walk-through of the realistic V1 → V2 lifecycle (V1 originally has no `NewVersion` reference; the `ContinueAsNew(NewVersion = "...")` shown in this sample's `MigratingWorkflowV1` is a single-line surgical hotfix you deploy when you're ready to migrate in-flight instances), see the ["Lifecycle of V1" and "What if I add v3 tomorrow?" sections in the EntityWithVersionedOrchestrationSample README](../EntityWithVersionedOrchestrationSample/README.md#lifecycle-of-v1-how-the-samples-v1-came-to-be). The same pattern and replay-safety argument apply to `MigratingWorkflow` here.
