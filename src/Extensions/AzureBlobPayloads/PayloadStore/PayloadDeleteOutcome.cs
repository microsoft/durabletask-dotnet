...
public enum PayloadDeleteOutcome
{
    ...
    /// This means the delete was accepted, not that the underlying bytes were reclaimed: if the container has
    /// blob versioning or soft delete enabled, the prior version or the soft-deleted blob is retained until a
    /// lifecycle-management policy or the configured retention period removes it.
    /// </summary>
    Deleted,
