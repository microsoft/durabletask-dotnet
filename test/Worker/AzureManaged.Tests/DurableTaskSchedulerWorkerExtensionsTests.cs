...
public class DurableTaskSchedulerWorkerExtensionsTests
{
    ...
    public async Task UseDurableTaskScheduler_OptionsChange_CreatesNewChannel()
    {
        ...
        services.PostConfigureAll<DurableTaskSchedulerWorkerOptions>(options =>
        {
            options.EndpointAddress = currentEndpoint;
        });

        TestOptionsChangeTokenSource<DurableTaskSchedulerWorkerOptions> changeTokenSource = new();
        services.AddSingleton<IOptionsChangeTokenSource<DurableTaskSchedulerWorkerOptions>>(changeTokenSource);

        await using ServiceProvider provider = services.BuildServiceProvider();
        IOptionsMonitor<GrpcDurableTaskWorkerOptions> optionsMonitor = provider.GetRequiredService<IOptionsMonitor<GrpcDurableTaskWorkerOptions>>();
        GrpcDurableTaskWorkerOptions initialOptions = optionsMonitor.Get(Options.DefaultName);
        ...
        GrpcDurableTaskWorkerOptions updatedOptions = optionsMonitor.Get(Options.DefaultName);
