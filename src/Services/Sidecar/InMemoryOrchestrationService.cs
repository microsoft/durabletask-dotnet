...
/// <summary>
/// In-memory implementation of the Durable Task backend storage provider.
/// </summary>
    ...
    IOrchestrationServicePurgeClient
{
    ...
    int IOrchestrationService.MaxConcurrentTaskOrchestrationWorkItems => Environment.ProcessorCount;

    int IOrchestrationService.MaxConcurrentTaskActivityWorkItems => Environment.ProcessorCount;
    ...
    {
        this.logger.AbandoningTaskActivityWorkItem(workItem.Id);
    ...
    public async Task<TaskOrchestrationWorkItem> LockNextTaskOrchestrationWorkItemAsync(
        ...
        CancellationToken cancellationToken)
    {
        var (instanceId, runtimeState, messages) = await this.instanceStore.GetNextReadyToRunInstanceAsync(
            cancellationToken);
    ...
    class InMemoryInstanceStore(ILogger logger)
    {
        ...
        public bool TryGetHistory(string instanceId, [NotNullWhen(true)] out List<HistoryEvent>? history)
        {
            if (!this.store.TryGetValue(instanceId, out SerializedInstanceState? state))
            {
                history = null;
            ...
            lock (state)
            {
                history = state.HistoryEventsJson.Select(e => e!.GetValue<HistoryEvent>()).ToList();
                return true;
            }
        }
        ...
        public void AddMessage(TaskMessage message)
        {
            ...
            lock (state)
            {
                ...
                else if (state.IsCompleted)
                {
                    ...
                    this.logger.DroppedMessageForCompletedOrchestration(
                        instanceId,
            ...
            List<OrchestrationState> results = this.store
                ...
                .Where(item =>
                {
                    ...
                    if (query.InstanceIdPrefix != null &&
                        !statusRecord.OrchestrationInstance.InstanceId.StartsWith(
                            query.InstanceIdPrefix,
