...
class SingleLockedTransfer : Test
{
    ...
    {
        registry.AddOrchestrator<LockedTransferOrchestration>();
    }

    public class LockedTransferOrchestration : TaskOrchestrator<EntityInstanceId[], int[]>
    {
        public override async Task<int[]> RunAsync(TaskOrchestrationContext context, EntityInstanceId[] entities)
        {
            var from = entities![0];
            var to = entities![1];
