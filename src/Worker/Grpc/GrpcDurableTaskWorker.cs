...
{
    readonly GrpcDurableTaskWorkerOptions options = Check.NotNull(options).Get(name);
    readonly IServiceProvider services = Check.NotNull(services);
    readonly ILoggerFactory loggerFactory = Check.NotNull(loggerFactory);
    readonly ILogger logger = loggerFactory.CreateLogger("Microsoft.DurableTask");
