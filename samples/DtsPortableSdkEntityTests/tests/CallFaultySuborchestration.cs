...
class CallFaultySuborchestration : Test
{
    ...
    {
        registry.AddOrchestrator<FaultySuborchestration>();
        registry.AddOrchestrator<CallFaultySuborchestrationOrchestration>();
    }

    class FaultySuborchestration : TaskOrchestrator<bool, string>
    {
        public override Task<string> RunAsync(TaskOrchestrationContext context, bool nested)
        {
            if (!nested)
            {
    ...
    class CallFaultySuborchestrationOrchestration : TaskOrchestrator<bool, string>
    {
        public override async Task<string> RunAsync(TaskOrchestrationContext context, bool nested)
        {
            ...
            catch (TaskFailedException taskFailedException)
            {
                ...
                if (!nested)
                {
                    ...
                    Assert.Contains(nameof(FaultySuborchestration.MethodThatThrowsException), taskFailedException.FailureDetails.StackTrace);
                }
                else
                {
                    Assert.Equal("KABOOOOOM", taskFailedException.FailureDetails.ErrorMessage);
                    Assert.Contains(nameof(FaultySuborchestration.MethodThatThrowsNestedException), taskFailedException.FailureDetails.StackTrace);
                    ...
                    Assert.Contains(nameof(FaultySuborchestration.MethodThatThrowsException), taskFailedException.FailureDetails.InnerFailure.StackTrace);
                }
            }
            catch (Exception e)
            {
